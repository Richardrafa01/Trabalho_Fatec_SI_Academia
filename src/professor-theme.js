...
const professorSidebarLinks = [
  { label: "Painel", href: "/professor/menu.html", icon: "home" },
  { label: "Alunos", href: "/professor/menu.html#alunos", icon: "users" },
  { label: "Presencas", href: "/professor/menu.html#aulas", icon: "check" },
  { label: "Treinos", href: "/professor/menu.html#treinos", icon: "clipboard" },
  { label: "Config", href: "/professor/configuracoes.html", icon: "settings" },
];
...
const iconPaths = {
  ...
  clipboard: '<path d="M9 5h6" /><path d="M9 3h6v4H9z" /><path d="M5 5h2" /><path d="M17 5h2v16H5V5" /><path d="M8 12h8" /><path d="M8 16h6" />',
...
function setupProfessorSidebar() {
  if (document.querySelector(".app-sidebar")) return;

  const storageKey = "professor-sidebar-collapsed";
  ...
  aside.innerHTML = `
    <div class="app-sidebar-brand">
      ...
      <strong class="app-sidebar-title">Professor</strong>
      <button class="app-sidebar-toggle" type="button" aria-label="Fechar menu" aria-expanded="${String(!isCollapsed)}">
        ${iconSvg("menu")}
      </button>
    </div>
    <nav class="app-sidebar-nav" aria-label="Navegacao principal do professor"></nav>
  `;

  const nav = aside.querySelector(".app-sidebar-nav");
  professorSidebarLinks.forEach(({ label, href, icon }) => {
    const link = document.createElement("a");
...
function setupProfessorContextNav() {
  ...
  nav.innerHTML = `
    <div class="app-context-nav-inner">
      <div>
        <p class="app-context-nav-title">Painel do professor</p>
        <p class="app-context-nav-subtitle">${professorPages[currentPath()] ?? "Rotina de atendimento"}</p>
      </div>
    </div>
  `;
